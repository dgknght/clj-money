    ...
    (swap! page-state
           ...
           (update-in account
                      ...
                      (comp (:commodities @page-state)
                            :id)))
    ; TODO: Maybe this belongs in a watch?
    (if (system-tagged? account :tradable)
  ...
  [:tr.align-middle
   ...
   [:td.text-center.d-none.d-md-table-cell
    [forms/checkbox-input page-state [:bulk-edit :account-ids] {:no-bootstrap? true
                                                                ...
                                                                :value id}]]
   [:td.text-center [account-row-buttons account page-state]]])
...
(defn- lot-row
  ...
  (let [g-l (- (* (:price/value latest-price)
               ...
               (* (:lot/purchase-price lot)
                  (:lot/shares-owned lot)))
        id (:id lot)
        audit-history (get-in @page-state [:lot-audit-histories id])
        expanded? (contains? (:lot-audit-expanded @page-state) id)]
    ^{:key (str "lot-" id)}
    [:tr
        ...
        (cond
          ...
          (->> @held-lots
               (sort-by (comp serialize-local-date
                              :lot/purchase-date))
               (map #(lot-row % @latest-price @gain-loss page-state))
               doall)
