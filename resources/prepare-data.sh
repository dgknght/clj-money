...
sed "$(grep -n \<gnc:pricedb target/sample-data.xml | cut -d ':' -f 1),$(grep -n \/gnc:pricedb target/sample-data.xml | cut -d ':' -f 1)d" target/sample-data.xml > target/sample-data-without-prices.xml
