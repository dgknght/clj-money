...
psql -U app_user --password please01 -h localhost -c "delete from accounts where commodity_id in (select id from commodities where name = 'Vanguard Moderate Age-Based Option') and id not in (select account_id from transaction_items);" -d money_development
echo "Done."
