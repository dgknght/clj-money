...
(deftest a-user-can-retrieve-future-dated-transaction-items
  (with-context future-context
    (let [future-date (t/plus (t/local-date) (t/months 1))
      ...
      (-> (request :post (path :api
                   ...
                   :user (find-user "john@doe.com")
                   :content-type "application/edn"
                   :body #:transaction{:description "Credit card payment"
                                       :transaction-date future-date
                                       :items [#:transaction-item{:action :credit
                                               ...
                                               #:transaction-item{:action :debit
                                                                   ...
                                                                   :quantity 200M}]})
          app)
      (let [response (-> (request :get (str (path :api
                                                  :accounts
                                                  (:id checking)
                                                  :transaction-items)
                                           "?"
                                           (map->query-string
                                             {:transaction-date-on-or-after "2015-01-01"
                                              :transaction-date-before (str (t/plus future-date (t/days 1)))}))
                         :content-type "application/edn"
                         :user (find-user "john@doe.com"))
                        app
                        parse-body)]
        (is (http-success? response))
        (is (seq-of-maps-like? [{:transaction/transaction-date future-date
                                ...
                                {:transaction/transaction-date (t/local-date 2015 1 1)
                                 ...
                                 :transaction-item/action :debit}]
                               (:parsed-body response))
            "Future-dated and past transaction items are both returned")))))
