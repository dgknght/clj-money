        ...
        (testing "before the start date"
          (is (empty? (t/do-at*
                        ...
                        #(st/next-transaction-dates
                           (dissoc sched-trx :scheduled-transaction/last-occurrence))))))
        (testing "after the start date"
          #_(testing "before the transaction date"
            (is (= expected
                   (t/do-at*
          ...
          (testing "after the transaction date"
            (is (= expected
                   (t/do-at*
                     ...
                     #(st/next-transaction-dates sched-trx))))))))
    #_(testing "irregular last day of month"
      (let [sched-tran (-> sched-trx
        ...
        #_(testing "after the start date"
          (testing "before the transaction date"
            (is (= expected
                   (t/do-at*
...
;     (testing "start date does not match the spec"
;       (is (= expected
;              (t/do-at* (t/date-time 2021 03 03)
;                (st/next-transaction-dates
;                  (assoc sched-trx
;                         :scheduled-transaction/start-date
;                         (t/local-date 2020 3 1)))))))
;     (testing "before the start date"
;       (is (empty? (t/do-at* (t/date-time 2019 03 01)
;                     (st/next-transaction-dates
;                       (dissoc sched-trx :scheduled-transaction/last-occurrence))))))
;     (testing "after the start date"
;       (testing "before the transaction date"
;         (is (= expected
;                (t/do-at* (t/date-time 2021 02 25)
;                  (st/next-transaction-dates sched-trx)))))
;       (testing "on the transaction date"
;         (is (= expected
;                (t/do-at* (t/date-time 2021 03 01)
;                  (st/next-transaction-dates sched-trx)))))
;       (testing "after the transaction date"
;         (is (= expected
;                (t/do-at* (t/date-time 2021 03 07)
;                  (st/next-transaction-dates sched-trx))))))
;     (testing "close proximity"
;       (let [result (t/do-at* (t/date-time 2021 03 05)
;                      (st/next-transaction-dates
;                        (update-in sched-trx
...
;          (t/do-at* (t/date-time 2020 03 03)
...
;   (t/do-at* (t/date-time 2016 02 02)
